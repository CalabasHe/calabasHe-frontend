  ...
  return (
    <>
      {facilities.map((facility) => (
        <div
          key={facility.id}
          className="cursor-pointer bg-white lg:hover:scale-[1.02] duration-300 mt-4 border border-black shadow-md h-40 sm:h-36 md:h-44 lg:h-52 rounded-md md:rounded-xl w-[98%] max-w-[380px] md:w-[85%] md:max-w-[800px] sm:max-w-[600px] p-1 lg:px-4 flex gap-4 md:gap-6 min-[400px]:gap-6"
        >
          ...
          <div className="lg:self-center grow flex flex-col sm:flex-row justify-between py-1">
            <div className="px-1 mb-auto overflow-hidden">
              <h2 className="text-sm  md:text-base font-bold  overflow-hidden text-wrap line-clamp-1">
                {facility.name }
              </h2>
              <p className="text-[10px] text-xs md:text-sm font-thin whitespace-nowrap overflow-hidden text-ellipsis">
                {facility.type}
              </p>
            ...
            <div className="mt-auto sm:hidden">
              ...
              {/* View Profile button */}
              <Link to={`/facilities/${facility.type.toLowerCase()}s/${facility.slug}`} className="block w-full">
            ...
            <svg className="translate-x-2  md:w-7 md:h-7" width="45" height="43" viewBox="0 0 45 43" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <p className="text-center font-semibold text-sm lg:text-base">Do you want to read the {facility.type.toLowerCase()}&apos;s reviews?</p>
